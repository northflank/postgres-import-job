...
if [ "${SKIP_MATVIEW}" == "true" ]; then
  pg_restore -l -Fc ${BASE_PATH}/dump | sed '/MATERIALIZED VIEW/d' > ordered.lst
else
  pg_restore -l -Fc ${BASE_PATH}/dump | sed '/MATERIALIZED VIEW DATA/d' > ordered.lst
fi
pg_restore -L ordered.lst ${RESTORE_FLAGS} -d "${DST_POSTGRES}" ${BASE_PATH}/dump
if [ "${DO_REFRESH}" == "true" ] && [ "${SKIP_MATVIEW}" != "true" ]; then
  pg_restore -l -Fc ${BASE_PATH}/dump | grep 'MATERIALIZED VIEW DATA' > refresh.lst
